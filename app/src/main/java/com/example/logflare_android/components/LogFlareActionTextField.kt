...
fun LogFlareActionTextField(
    ...
    val inputEnabled = enabled && state != LogFlareActionTextFieldState.Disabled
    val readOnly = state == LogFlareActionTextFieldState.Saved

    val borderColor = when (state) {
        LogFlareActionTextFieldState.Error -> colors.red.default
        LogFlareActionTextFieldState.Success -> colors.primary.default
        LogFlareActionTextFieldState.Validating -> colors.primary.default
        LogFlareActionTextFieldState.Saved -> colors.neutral.s50
        LogFlareActionTextFieldState.Disabled -> colors.neutral.s30
        else -> if (isFocused) colors.primary.default else colors.neutral.s40
    }

    val inputBackground = when (state) {
        LogFlareActionTextFieldState.Saved, LogFlareActionTextFieldState.Disabled -> colors.neutral.s10
        else -> colors.neutral.white
    }

    val placeholderColor = if (inputEnabled) colors.neutral.s60 else colors.neutral.s50
    val textColor = if (inputEnabled) colors.neutral.s90 else colors.neutral.s60

    val actionButtonEnabled = actionEnabled && inputEnabled &&
        state != LogFlareActionTextFieldState.Saved &&
        state != LogFlareActionTextFieldState.Validating

    val buttonBackground = when {
        state == LogFlareActionTextFieldState.Validating -> colors.primary.default
        actionButtonEnabled -> colors.primary.default
        else -> colors.neutral.s30
    }

    val buttonContentColor = when {
        state == LogFlareActionTextFieldState.Validating -> colors.neutral.white
        actionButtonEnabled -> colors.neutral.white
    ...
    Column(modifier = modifier) {
        Row(
            ...
            Surface(
                onClick = { if (actionButtonEnabled) onActionClick?.invoke() },
                enabled = actionButtonEnabled && onActionClick != null,
                shape = AppTheme.radius.medium,
